  ...
  /* ===================== QA 회차별 "자동 정리" (AI 미사용, 규칙 기반) ===================== */
  // AI 요점 정리(qaOrganizeRoundItems)는 회차마다 버튼을 눌러 외부 서버(Groq)를 호출해서
  // 정리해준다. 여기서는 그거랑 별개로, 같은 회차 안에서 네트워크 호출 없이 브라우저 안의
  // 규칙만으로 즉시 정리해서 보여준다. 원문을 무작정 다 가져오는 게 아니라:
  // - 칭찬/문제없음류의 코멘트(실제로 고칠 게 없는 내용)는 제외한다.
  // - "OOO의 경우"처럼 특정 상담 내용이 아니라 가이드라인 자체를 조건문으로 다시 설명한
  //   문장(=실제 피드백이 아닌 일반 규정 설명)도 제외한다.
  // - 남는 코멘트만, 원문에 붙어있던 "-"·번호 같은 군더더기 기호를 정리해서 짧게 보여준다.
  ...
  // 글자 수로 뚝 잘라 "…"만 붙이면 문장 중간에서 끊겨 어색해 보이므로, 문장(마침표·느낌표·
  // 물음표 기준) 단위로 끊어서 제한 길이 안에 들어가는 "완결된 문장"까지만 이어붙인다.
  // 그렇게 담아도 첫 문장 자체가 너무 길면, 그때만 어절(공백) 단위로 자연스럽게 줄인다.
  function qaSummarizeFeedback(text, maxLen) {
    ...
    for (const s of sentences) {
      ...
      if (result && candidate.length > limit) break; // 이미 담긴 문장이 있으면 다음 문장은 넘어가지 않고 멈춘다
      result = candidate;
      if (result.length >= limit) break;
    }
    if (!result) result = sentences[0];
    if (result.length > limit) {
      // 문장 하나가 이미 제한보다 길 때만: 어절 단위로 줄이고, 문장이 안 끝났을 때만 말줄임표를 붙인다.
      const words = result.slice(0, limit + 30).split(" ");
      words.pop();
      result = words.join(" ").trim() || result.slice(0, limit).trim();
      if (!/[.!?]$/.test(result)) result += "…";
    }
    return result;
  }
  // 한 회차의 items 중 실제로 개선이 필요한 코멘트만 걸러 "[가이드라인]\n- 코멘트" 형태의
  // 텍스트로 정리한다. qaFormatSummaryHtml에 그대로 넣으면 AI 요약과 같은 모양으로 보인다.
  function qaBuildLocalRoundSummaryText(items) {
    ...
    return kept
      .map((it) => {
        ...
        const cleaned = qaSummarizeFeedback(qaCleanFeedbackForDisplay(it.feedback), 90);
        return `${label}\n- ${cleaned}`.trim();
      })
      .join("\n");
  }
  ...
  function openQADetailModal(agentId) {
          ...
          ${detail.rounds.map((round, idx) => {
            ...
            if (isPerfect) {
              ...
              <div class="qa-round-local">
                <button type="button" class="qa-round-raw-toggle" data-qa-local-toggle="${idx}">자동 정리 보기</button>
    ...
    // "자동 정리" 토글: 버튼을 누르면(첫 클릭 때만) 그 자리에서 즉시 규칙 기반으로 정리해서
    // 보여주고, 다시 누르면 접힌다. 네트워크 호출이 없어 AI 요약과 달리 기다릴 필요가 없다.
    overlay.addEventListener("click", (e) => {
      ...
      localBtn.textContent = opening ? "자동 정리 접기" : "자동 정리 보기";
    });
