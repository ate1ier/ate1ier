  ...
  function renderQAPage(root) {
    ...
    root.innerHTML = `
      ...
      <div class="qa-dropzone" id="qa-dropzone">${ICON_UPLOAD} 여기로 엑셀 파일을 끌어다 놓아도 업로드돼요 (여러 개 한꺼번에 가능)</div>
    ...
    document.getElementById("qa-excel-input").onchange = (e) => {
      ...
      e.target.value = "";
      qaHandleExcelFiles(files).catch((err) => {
        // qaHandleExcelFiles 내부에서 못 잡은 예외가 있으면(원래는 콘솔에도 안 뜨고 조용히
        // 사라졌던 경우) 여기서 반드시 콘솔+팝업으로 드러나게 한다.
        console.error("[QA 업로드] 처리 중 예상치 못한 오류:", err);
        alert(`엑셀 업로드 중 예상치 못한 오류가 발생했어요.\n\n${err && err.message ? err.message : err}\n\nF12 콘솔에 자세한 내용이 남았어요.`);
      });
    };

    // ----- 드래그앤드롭 업로드 -----
    // 파일 선택 대화상자(input.click())가 VDI 등 일부 환경에서 파일을 0개로
    // 전달하는 문제가 있어서, 우회 경로로 드래그앤드롭도 지원한다.
    const qaDropzone = document.getElementById("qa-dropzone");
    const qaHandleDrop = (fileList) => {
      console.log("[QA 업로드] 드래그앤드롭으로 전달된 파일 수:", fileList ? fileList.length : 0);
      qaHandleExcelFiles(fileList).catch((err) => {
        console.error("[QA 업로드] 드래그앤드롭 처리 중 예상치 못한 오류:", err);
        alert(`엑셀 업로드 중 예상치 못한 오류가 발생했어요.\n\n${err && err.message ? err.message : err}\n\nF12 콘솔에 자세한 내용이 남았어요.`);
      });
    };
    ["dragenter", "dragover"].forEach((evt) => {
      qaDropzone.addEventListener(evt, (e) => {
        e.preventDefault();
        e.stopPropagation();
        qaDropzone.classList.add("qa-dropzone-active");
      });
    });
    ["dragleave", "dragend"].forEach((evt) => {
      qaDropzone.addEventListener(evt, (e) => {
        e.preventDefault();
        e.stopPropagation();
        qaDropzone.classList.remove("qa-dropzone-active");
      });
    });
    qaDropzone.addEventListener("drop", (e) => {
      ...
      if (!files || !files.length) {
        console.warn("[QA 업로드] 드롭했지만 dataTransfer.files가 비어있어요.", dt);
        flashQAStatus("파일을 인식하지 못했어요. 엑셀 파일 자체를 끌어다 놔주세요.");
        return;
      }
      qaHandleDrop(files);
    });
    // 드롭존 밖(브라우저 창 전체)에 실수로 떨어뜨려서 파일이 통째로 열려버리는 것 방지.
